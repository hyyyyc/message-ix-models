...
def report_iam_definition(
    ...
    if not suban:
        ...
        key = rep_dm.add("demand", "as_pyam", rename=rename,
    ...
    if not suban:
        ...
        for vs in vars_dic:
            ...
            df = (
                df.groupby(
                    ["model", "scenario", "variable", "subannual", "year"])
                .apply(
                    lambda x: x.assign(
                        region=(
                            x["reg2"]
...
def multiply_electricity_output_of_hydro(
    ...
    for var in elec_hydro_var:
        ...
        if not match:
            log.error(
                f"Could not extract hydro technology name from variable: {var}")
            continue
        ...
        if tech.empty:
            log.error(
                f"No performance data found for technology: {tech_suffix}")
            continue
...
def get_population_data(sc: Scenario, reg: str) -> pd.DataFrame:
    ...
    for ur in ["urban", "rural"]:
        try:
            ...
            if pop_data.empty:
                log.warning(
                    f"No Population|{ur.capitalize()} data found in scenario")
                continue
            ...
            pop_data["region"] = (
                pop_data["region"].map(
                    population_to_r12_map).fillna(pop_data["region"])
            )
            ...
            log.warning(
                f"Failed to retrieve Population|{ur.capitalize()} data: {e}")
            continue
    ...
    if future_pop.empty:
        ...
        log.info(
            f"Population data available through year {population_data.year.max()}")
...
def get_rates_data(reg: str, sdgs: bool = False) -> pd.DataFrame:
    ...
    rates_data = (
        df_rate.groupby(["year", "variable", "region"])[
...
def aggregate_totals(result_df: pd.DataFrame) -> list[pd.DataFrame]:
    ...
    sani = result_df[
        result_df.variable.str.contains(
            r"Population\|Sanitation Access", regex=True)
    ]
    if not sani.empty:
...
def pop_water_access(sc: Scenario, reg: str, sdgs: bool = False) -> pd.DataFrame:
    ...
    all_years = sorted(set(rates_data.year.unique())
                       | set(pop_data.year.unique()))
    all_regions = sorted(
    ...
    for region in all_regions:
        for year in all_years:
            ...
            urban_val, rural_val = get_population_values(
    ...
    if not result_df.empty:
        ...
        if world_totals:
            result_df = pd.concat(
                [result_df] + world_totals, ignore_index=True)
...
def compute_cooling_technologies(
    ...
    fresh_return_emissions = report_iam.filter(
        variable="emis|fresh_return|*").variable

    # Cooling investments
    cooling_saline_inv = report_iam.filter(
        variable="inv cost|*saline").variable
    cooling_air_inv = report_iam.filter(variable="inv cost|*air").variable
    cooling_ot_fresh = report_iam.filter(
        variable="inv cost|*ot_fresh").variable
    cooling_cl_fresh = report_iam.filter(
        variable="inv cost|*cl_fresh").variable

    # Hydro electricity calculations
    elec_hydro_var = report_iam.filter(
        variable="out|secondary|electr|hydro*").variable
    report_iam = multiply_electricity_output_of_hydro(
        elec_hydro_var, report_iam)
...
def report_ibwt(report_iam: pyam.IamDataFrame) -> pd.DataFrame:
    ...
    # couldn't work, the node_loc in output for ibwt is still "node_in"
    ...
    map_agg_pd = pd.DataFrame(
        [
            ...
            ["Investment|Interbasin Water Transfer",
                ibwt_var_dict["inv_11"] +
                ibwt_var_dict["inv_12"] +
                ibwt_var_dict["inv_13"] +
                ibwt_var_dict["inv_14"] +
                ibwt_var_dict["inv_15"] +
                ibwt_var_dict["inv_16"] +
                ibwt_var_dict["inv_17"] +
                ibwt_var_dict["inv_18"], "million US$2010/yr"],
...
def report(
    ...
    """
    log.info(
        f"Regions given as {reg}; no warranty if it's not in ['R11','R12']")
    # Generating reporter
    ...
    # if subannual, get and subsittute variables
    report_iam = report_iam_definition(
        sc, rep, df_dmd, rep_dm, report_df, suban)
    ...
    extract_sw = [
        "in|water_avail_basin|surfacewater_basin|extract_surfacewater|M1"]

    extract_gw = [
        "in|water_avail_basin|groundwater_basin|extract_groundwater|M1"]
    extract_fgw = [
        "out|water_supply_basin|freshwater_basin|extract_gw_fossil|M1"]
    ...
    rural_mwdem_unconnected = [
        "out|final|rural_disconnected|rural_unconnected|M1"]
    rural_mwdem_unconnected_eff = [
        "out|final|rural_disconnected|rural_unconnected|Mf"]
    ...
    urban_mwdem_unconnected = [
        "out|final|urban_disconnected|urban_unconnected|M1"]
    urban_mwdem_unconnected_eff = [
        "out|final|urban_disconnected|urban_unconnected|Mf"]
    ...
    industry_mwdem_unconnected_eff = [
        "out|final|industry_mw|industry_unconnected|Mf"]
    ...
    extract_saline_region = [
        "out|saline_supply|saline_ppl|extract_salinewater_cool|M1"]
    ...
    urban_treated_wstwtr = [
        "in|water_treat|urban_collected_wst|urban_recycle|M1"]
    rural_treated_wstwtr = [
        "in|water_treat|rural_collected_wst|rural_recycle|M1"]

    urban_wstwtr_recycle = [
        "out|water_supply_basin|freshwater_basin|urban_recycle|M1"]
    rural_wstwtr_recycle = [
        "out|water_supply_basin|freshwater_basin|rural_recycle|M1"]

    urban_transfer = ["in|water_supply_basin|freshwater_basin|urban_t_d|M1"]
    urban_transfer_eff = [
        "in|water_supply_basin|freshwater_basin|urban_t_d|Mf"]
    rural_transfer = ["in|water_supply_basin|freshwater_basin|rural_t_d|M1"]
    rural_transfer_eff = [
        "in|water_supply_basin|freshwater_basin|rural_t_d|Mf"]
    ...
    map_agg_pd = pd.DataFrame(
        [
            ...
            ["Water Withdrawal|Irrigation_in", irr_c + irr_o + irr_s, "MCM/yr"],
            ...
            ["Water Withdrawal|Irrigation_out", irr_water, "MCM/yr"],
            [
    ...
    map_agg_pd_ibwt = report_ibwt(report_iam)
    map_agg_pd = pd.concat([map_agg_pd, map_agg_pd_ibwt], ignore_index=True)

    # Add water prices, ad-hoc procedure
    wp = sc.var(
        "PRICE_COMMODITY", {"commodity": [
            "urban_mw", "rural_mw", "freshwater"]}
    )
    wp["value"] = wp["lvl"]
    ...
    col_ex = report_iam.as_pandas(
    ).columns[report_iam.as_pandas().columns != "exclude"]
    wp = wp[col_ex]
    ...
    map_node_dict = map_node.groupby("node_parent")[
    ...
    for index, row in map_agg_pd.iterrows():
        ...
        report_iam.aggregate(
            row["names"], components=row["list_cat"], append=True)
    ...
    report_iam.filter(variable=varsexclude, unit="unknown",
...
# Connect to a db
mp = ixmp.Platform(name="ixmp_dev", jvmargs=["-Xmx20G"])

model = "MESSAGE_GLOBIOM_SSP2_v6.1"
scen = "FullScale_baseline_baseline_nexus_7p0_high"
